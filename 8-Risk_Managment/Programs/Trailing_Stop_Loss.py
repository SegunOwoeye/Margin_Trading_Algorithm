...
class Monitor:
    ...
    def manage_trade(self):
        ...
        for i in range(len(data[1])):
            if data[1][i] == "Closed": # Filtering Out Closed Positions
        ...
        for i in range(len(data[0])):
            # Trailing Percentage
            trailing_percentage = round((data[5][i]-data[9][i])/data[9][i] * 2, 4)
            # [4.3.0] Getting Profit Percentage
            profit_percentage = (current_price - data[9][i])/data[9][i] * 100
            ...
            if profit_percentage > 0:
                if data[2][i] == "LONG": # LONG
                    # new_stop_loss = entry_price + (current_price - entry_price) * (1 - trailing_percent)
                    new_stop_loss = data[9][i] + (current_price - data[9][i])*(1-trailing_percentage)
                    stop_loss = max(new_stop_loss, data[6][i])
                else: # SHORT
                    # new_stop_loss = entry_price - (entry_price - current_price) * (1 - trailing_percent)
                    new_stop_loss = data[9][i] - (data[9][i] - current_price) * (1-trailing_percentage)
                    stop_loss = min(data[6][i], new_stop_loss)
            ...
            main = order(trading_pair=self.trading_pair, exchange_name=self.exchange_name, flag=self.flag, side=data[2][i], asset_equity=data[3][i], 
                         chart_interval=self.chart_interval, db_name=self.db_name, TP=data[5][i], Stop_Loss=stop_loss, Stop_Limit=stop_limit,
                         OrderID=[data[10][i],data[11][i]])
