...
""" [1] Importing Custom indicators """
path.append("6-DynamicBacktesting/Programs/custom_bt")

from tick_percentage_bt import tick_pReturns


""" [2] Gathering price data """
# [2.1] Gathering Price data when there is a file available
...
# [2.2.1] Calculating the number of intervals -> BT
...
# [2.2.2] Gathering the dates based on number of intervals -> BT
...
# [2.2.3] Gathering the price data based on number of intervals -> BT    
...
# Custom
def get_signals(trading_pair, chart_interval, p_threshold, hm_spaceing, metric):
    ...
    file_name = f"6-DynamicBacktesting/data_gathered/{trading_pair}/raw_output{chart_interval}.csv"
    price_data = pd.read_csv(file_name)[["open_time", "close", "open"]]
    ...
    tick_return = tick_pReturns.run(close=close_price, open=open_price, p_threshold=p_threshold)
    
    return tick_return.value



# Custom
def bt_strategy2(trading_pair, chart_interval, p_threshold, hm_spaceing, metric, flag ="optimise: TP/SL"):#flag=None):
    #Gathering signals
    #Short
    short_signals = get_signals(trading_pair, chart_interval, p_threshold, hm_spaceing, metric)
    short_tick = short_signals

    # Taking the sum of the signals, should be 1 to buy or -1 to sell
    short_signal_sum = short_tick
    
    #Long
    long_signals = get_signals(trading_pair, chart_interval, p_threshold, hm_spaceing, metric)
    
    long_tick = long_signals

    # Taking the sum of the signals, should be 1 to buy or -1 to sell
    long_signal_sum = long_tick
    
    #long
    long_entries = long_signal_sum == 1.0

    #short
    short_entries = short_signal_sum == -1.0
    ...
    if flag == "optimise: rsi":
        ...
        for item in short_profit_return:
            if True:#item['Returns'] < average_short_returns or item['TP']< item['SL']:# or (item['Returns'] < short_benchmark):
                pass
            else:
        ...
        for item in long_profit_return:
            if True:#(item['Returns'] < average_long_returns) or (item['TP']< item['SL']):# or (item['Returns'] < long_benchmark):
                pass
            else:
...
def run(trading_pair, chart_interval):
    ...
    while True: 
        if exists(file_name) == True: # Performs backtest if datafile is available
            bt_strategy2(trading_pair, chart_interval, p_threshold,
