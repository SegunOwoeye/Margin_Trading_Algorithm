...
def get_historical_kline_data(exchange_pair, interval, limit): 
    ...
    path = f"/v5/market/kline?category=linear&symbol={exchange_pair}&interval={interval}&limit={limit}"
    url = host + path
    ...
    return last_recent_list #Newest clines come in at the bottom
...
def creating_db_file(exchange_pair, exchange_name, interval):
    ...
    file_name = f"1-DataGathering/data_gathered/{trading_pair}_data/Historical_Klines/" + str(date) + exchange_name + exchange_pair + "interval=" + str(interval) + "market_data.db"
