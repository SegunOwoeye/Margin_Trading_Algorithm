"""
KALMAN FILTER:
    Shows a smoothed version of the raw price data, where the Kalman filter attempts to remove random fluctuations 
    and highlight the underlying trend or pattern.
...
"""
da = price_data("BTCUSDT", "5m")
# Extract the 'Close' prices as a NumPy array
real_prices = da.values[-100:]
...
def kalman_filter(prices):
    ...
    # State transition model (how the state evolves from one time step to the next)
    kf.F = np.array([[1, 1],
                    [0, 1]])

    # Observation model (how we observe the state)
    kf.H = np.array([[1, 0]])
...
"""
predicted_prices = kalman_filter(real_prices)
x_axis = np.linspace(1, len(predicted_prices), num=len(predicted_prices))
"""
...
"""
evaluation = evaluate_predictions(real_prices, predicted_prices)

for i in range(1,10):
    print(f"Real: {real_prices[-11+i]} | Predicted {predicted_prices[-11+i]}")
...
plt.plot(x_axis, real_prices, 'b-', label='real_prices')
plt.plot(x_axis, predicted_prices, 'r-', label='predicted_prices')
